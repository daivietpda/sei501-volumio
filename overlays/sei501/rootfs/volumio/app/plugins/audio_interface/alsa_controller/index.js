  ...
  if (process.env.MODULAR_ALSA_PIPELINE === 'true') {
    ...
    return promise
      ...
      .then((result) => {
        ...
        if (self.config.get('outputdevicecardname') === 'SEI501') {
          exec('/usr/local/sbin/sei501-audio-init', function (error) {
            if (error) {
              self.logger.warn('Cannot enable SEI501 audio routes: ' + error);
            }
          });
        }
        return result;
