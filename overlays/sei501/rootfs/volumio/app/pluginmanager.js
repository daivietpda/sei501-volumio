...
PluginManager.prototype.getAvailablePlugins = function () {
  ...
  function fetchAvailablePlugins () {
    self.logger.info('Fetching public plugins list from ' + url);
    unirest
      .get(url)
      .timeout(10000)
      .then(function (response) {
        self.logger.info('Public plugins response status=' + (response && response.status) + ' code=' + (response && response.statusCode) + ' body=' + (response && typeof response.body) + ' categories=' + (response && response.body && response.body.categories && response.body.categories.length));
        if (response && response.status === 200 && response.body && response.body.categories) {
          pushAvailablePlugins(response.body);
        } else {
          self.logger.error('Cannot download public plugins list');
